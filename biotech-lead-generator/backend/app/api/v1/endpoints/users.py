...
from app.schemas.token import APIKeyCreate, APIKeyResponse, APIKeyList
...
@router.get(
    ...
    description="Get user's usage statistics and quotas"
)
async def get_usage_stats(
    ...
    usage_stats = current_user.usage_stats or {}
    
    leads_created = usage_stats.get("leads_created_this_month", 0)
    limit = current_user.get_monthly_lead_limit()
    usage_percentage = (leads_created / limit * 100) if limit > 0 else 0
    
    return UserUsageStats(
        leads_created_this_month=leads_created,
        leads_limit_per_month=limit,
        searches_this_month=usage_stats.get("searches_this_month", 0),
        exports_this_month=usage_stats.get("exports_this_month", 0),
